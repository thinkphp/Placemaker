               ...
               if(!in_array($wi.'',$history)) {
                 ...
                 $currentloc = $foundplaces["woeid".$wi];
                        ...
                        if($r->text != '' && $currentloc['name'] != '' && $currentloc['lat'] != '' && $currentloc['lon'] != '') {
                           ...
                           $current = $wi.'|'.$currentloc['name'].'|'.$text.'|'.$currentloc['lat'].'|'.$currentloc['lon'];

                           $matches[] = $text;

                           $out .= '<td><input type="checkbox" checked="checked" name="collection[]" value="'.$current.'"></td>';

                           $out .= '<td>'.$r->text.'</td><td>'.str_replace(', ZZ','',$currentloc['name']).'</td>';
