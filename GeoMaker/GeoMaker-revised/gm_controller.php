<?php
     ...
     */
     ...
     if($_POST['stage'] == 'filter') {
              ...
              //if the user sent a URL then 
                if($_POST['url'] and $_POST['url'] != '') {
                                                    ...
                                                    if($places->channel->item){
                                                                ...
                                                                foreach($places->channel->item as $p){
                                                                                 ...
                                                                                 if($locs) {
                                                                                         ...
                                                                                         $content = '<h2><a href=\"'.$p->link.'\">'.cleanup($p->title).'</a></h2><div class=\"feedcontent\">'.cleanup($p->description).'</div>';

                                                                                         $out .= '<li><h3><a href="'.$p->link.'">'.stripslashes(cleanup($p->title)).'</a></h3><div class="yui-ge">'.'<div class="yui-u first">'.'<div class="feedcontent">'.stripslashes(cleanup($p->description)).'</div></div><div class="yui-u">';

                                                                                               foreach($locs->clplace as $pl){

                                                                                                        $name = str_replace(', ZZ','',$pl->clname);

                                                                                                        $current = $pl->clwoeId.'|'.$name.'|'.htmlentities($content).'|'. $pl->cllatitude.'|'.$pl->cllongitude.'|'.cleanup($p->title);

                                                                                                        $out .= '<div><input checked="checked" type="checkbox"'.' name="collection[]" value="'.$current.'">'.$name.'</div>';
                ...
                //if the user sent a message then
                ...
                }//end if $_POST['message']

      
                if((!isset($_POST['message']) and !isset($_POST['url'])) or ($_POST['message'] == '' and $_POST['url'] == '')) {
    ...
    if($_POST['stage'] == 'output') {
         ...
         foreach($_POST['collection'] as $c) {
                 ...
                 if(isset($_POST['rss'])) {
                      ...
                      $points[] = '{"lat":"'.$chunks[3].'","lon":"'.$chunks[4].'","content":"'.$chunks[2].'","title":"'.$chunks[1].': '.$chunks[5].'"}';
